...
def get_flops(model, imgsz=640):
    ...
    try:
        ...
        try:
            ...
            stride = max(int(model.stride.max()), 32) if hasattr(model, 'stride') else 32  # max stride  32  64
             ####----------------------------------------------------------------------------------------####
            if type(model.model[0]).__name__ == 'IN' :
...
def get_flops_with_torch_profiler(model, imgsz=640):
    ...
    if TORCH_2_0:
        ...
        im = torch.zeros((1, p.shape[1], 640, 640), device=p.device)  # input image in BCHW format
